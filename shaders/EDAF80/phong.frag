...
{	
	// Normal mapping
	vec3 normal_rgb = texture(normal_map, fs_in.texcoord).xyz;
	...
	vec3 diffuse_r = texture(texture_map, fs_in.texcoord).xyz * max(dot(N, L), 0.0);
	vec3 specular_r = texture(roughness_map, fs_in.texcoord).xyz * pow(max(dot(R, V), 0.0), shininess);
